	...
	if($clogin->_logeado()){
		...
		if(isset($_POST['id_nav'])) {
			...
			if(intval($permisos) > 0){
				...
				//------------------------------------------------------------------------
				$html = "<div id='frm-$id_navigator' data-op='".$id_operator."' data-nav='".$id_navigator."' data-table='".boolval($tabla)."'  class='frm frm-employees fn boxshadow mar10 oculto'>
					...
					$html.="<div id='set-departamento' data-departamento='$dep' data-ope='$id_operator' data-title='Seleccionar departamentos' data-panel='#frm-set-departamento'  class='fn tab panel bloque waves-effect unactive '>
							...
							$html.="<div id='frm-table' class='fs istab floL enlinea visible'>
											...
											$html.= "<div id='code' class='floL fs colm enlinea id'><span class='fs eti'>Codigo:</span> <!--<i class=' fa fa-1x fa-filter'></i>!--></div>
